...
public class PostchargeCheckBatchActivity extends BaseActivity implements ApiCallback {
    ...
    protected void initLayout(Bundle savedInstanceState) {
        setContentView(R.layout.activity_postchargecheckbatch);
    }

    @Override
    protected void init(Bundle savedInstanceState) {

        titleName.setText("充后验满批次");
        rightText.setVisibility(View.VISIBLE);
        rightText.setText("刷新");
        listAdapter = new PrechargeCheckBatchListAdapter(this);
        listview.setAdapter(listAdapter);
        userPresenter = new UserPresenter(this);
        userPresenter.getPostChargeDetectionBatchList();
    ...
    public <T> void successful(String api, T success) {
        ...
        if (api.equals("getPostChargeDetectionBatchList")) {
            listAdapter.deleteAllData();
            todayBottleCount.setText("今日验满：" + 0 + "（集格以集格内瓶数算）");
            ...
            if (datas != null && datas.size() > 0) {
                ...
                if (beans.size() > 0) {
                    ...
                    todayBottleCount.setText("今日验满：" + cyCount + "（集格以集格内瓶数算）");
                }
            }
        }
    }
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.back_img:
                ...
                break;
            case R.id.create:
                Intent intent = new Intent(PostchargeCheckBatchActivity.this, PostchargeCheckActivity.class);
